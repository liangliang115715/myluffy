	...
	<div class="shopping-cart-wrap">
        ...
        >
              ...
              <!--{{scope.row.relate_price_policy}}-->
			      	...
			      	<select v-model = 'scope.row.choosen_price_policy_id' @change="updatePricePolicyHandler(scope.row,scope.row.choosen_price_policy_id)" >
			      		<option v-for = '(item,index) in scope.row.relate_price_policy' :key = 'index'  :value = 'item.pk'>
			    ...
			    <el-table-column
			      ...
			      <template slot-scope='scope'>
			      	¥{{ scope.row.choosen_price }}
              <!--<p>¥{{ scope.row.relate_price_policy[scope.row.choosen_price_policy_id.toString()].price }}</p>-->
			      </template>
			    </el-table-column>
			 <el-table-column
			      fixed="right"
	...
	computed:{
		totalPrice(){
			...
			this.multipleSelection.forEach((item,index)=>{
				total += parseInt(item.choosen_price)
				// total += parseInt(item.relate_price_policy[item.choosen_price_policy_id.toString()].price)
			});
			return  total;
		},
	},
    ...
    account(){
      if (this.multipleSelection.length){
        ...
        this.multipleSelection.forEach((item,index)=>{
          arr.push({
          "course_id":item.id,
          "price_policy_id":item.choosen_price_policy_id
        })
      });
//
    ...
    updatePricePolicyHandler(item,price_policy_id){
        item.choosen_price = item.relate_price_policy[price_policy_id].price
